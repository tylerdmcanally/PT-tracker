...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.4.5 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline. Version 1.4.5 advances the Romanian deadlift to **125 lb total for 2 × 8** and uses `foundation-1.4.5`, which preserves the same work while adding the known M4 equipment and lane guidance. Never revise the older template in place.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.4.5**, effective **2026-08-21**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.4.5 leaves Days 1, 2, and 4 exactly as prescribed in v1.4.4 and changes Day 3 only. Romanian deadlift is 125 lb total for 2 × 8, goblet squat is 55 lb for 3 × 9, incline dumbbell press is 30 lb per hand for 3 × 10, one-arm dumbbell row is 45 lb for 3 × 11 per side, body-weight split squat is 2 × 10 per leg, and side plank is 3 × 40 seconds per side. Hammer curl is corrected to 20 lb per hand for 2 × 15. Overhead cable triceps extension is 88 lb for 2 × 15 on the same rope/cable setup, but that displayed stack number remains setup-specific rather than a universal adherence target. The progressive lower-body warm-up is approximately 10 minutes. The circuit remains exactly two rounds, Stage 3 remains unchanged, and the auxiliary microdose remains unchanged.
