...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.7 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline. Versions 1.4.5 through 1.4.8 use **125 lb total for 2 × 8** and `foundation-1.4.5`, which preserves the same circuit work while adding the known M4 equipment and lane guidance. Versions 1.4.9 through 1.5.1 use **135 lb total for 2 × 8** without changing the circuit. The September 3, 2026 workout remains an immutable version 1.5.1 snapshot; version 1.5.2 changed only Day 2, so its Day 3 prescription would have been identical. Versions 1.5.3 through 1.5.7 use **145 lb total for 2 × 8** while retaining `foundation-1.4.5`. Never revise an older template or saved snapshot in place.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.7**, effective **2026-09-14**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.7 changes only active Day 2. Hand-release push-ups are **5 × 11** at RPE 6–8 with five equal, clean, submaximal sets. Seated lat pulldown is the next increment above 176 lb on the same setup—approximately 187 displayed on the same 11-lb stack—for **3 × 8–10** at RPE 6–8, capped at 10 repetitions per set. Seated dumbbell overhead press remains **30 lb per hand for 3 × 9** at RPE 7–9 until all three sets are clean, with no load increase or make-up repetitions. Machine row is **110 displayed on the same confirmed setup for 3 × 9** at RPE 7–8. Cable lateral raise is the next same-setup increment above 33 displayed per side—approximately 44 if applicable—for **2 × 12–15** at RPE 7–8. Pec deck is the next same-setup increment above 77 displayed—approximately 88 if applicable—for **2 × 10–12** at RPE 7–9. Dead bug remains **3 × 10 each side**. Easy cardio remains **25–30 minutes at conversational RPE 4–5**, with no duration increase or make-up work. All displayed machine and cable loads remain setup-specific.

The completed September 13 Day 2 workout remains authoritative v1.5.6 history through its immutable embedded snapshot and results; normalization, editing, exports, local persistence, and Firebase round trips must never substitute v1.5.7 targets. Version 1.5.6 retired the active v1.5.5 **Day 1 — Illness Recovery** definition and selector option without creating a replacement or reusable template. The completed September 10 v1.5.5 workout remains authoritative through its immutable embedded snapshot, including `dayKey: day1IllnessRecovery`, `rotationDayKey: day1`, primary/advancing classification, stable `illnessReturnCheck` identity, shared exercise identities, and all saved results. Snapshot-backed retired session keys normalize and render without an active `SESSIONS` definition; the historical entry still exports, imports, persists locally, round-trips through Firebase, and counts as primary Day 1. Standard Days 1, 3, and 4, global Stage 4, the capped Day 3 circuit, TANK M4 Level 3 metadata, optional recovery, and the auxiliary microdose remain unchanged. September 3 and September 5 remain immutable v1.5.1 and v1.5.3 snapshots, any v1.5.4 workout remains immutable, and version 1.5.2 was never installed or synthesized in the public app. There is no automatic illness decision, weekday scheduling, third weekly run, or automatic progression.
...
Run training defines 12 possible stages, but the current stage is read-only and coach-directed. Version 1.5.7 holds active Stage 4 at 1:00 walk / 2:30 run × 6 on standard Days 1 and 4: six minutes walking plus fifteen minutes running for 21 programmed minutes. The post-strength Day 1 running segments begin around 5.5–5.6 mph, stay controlled through at least the first three rounds, and may move toward 5.7–5.8 mph only while RPE remains at or below 6 with relaxed stride and breathing. The fresh-leg Day 4 running segments begin around 5.7–5.8 mph and hold that range through at least round 3; move toward 5.9–6.0 only if RPE is at or below 5 through round 3 and reduce toward 5.5 if RPE exceeds 6. Do not advance beyond Stage 4 until two pain-free completions under this guidance land at RPE 5–6, including one fresh Day 4 without a late forced slowdown. Both runs prioritize duration and reserve rather than speed, total distance, or average pace. The retired historical illness-recovery snapshot contains no run. Every interval round begins with walking. The existing walk/run speed fields record actual speeds; completion never advances or regresses the program automatically. Historical workouts retain their saved prescription, guidance, and result fields.
...
The optional AFT Skill Microdose prescribes hand-release push-ups 3 × 4, front plank 3 × 20 seconds, and up to five minutes of gentle optional mobility at session RPE 3–4. It is the sole optional extra push-up/plank practice session in version 1.5.7. Historical v1.3 Day 3 bundles still satisfy their saved Monday–Sunday dose, but v1.4 and later Day 3 workouts cannot consume the standalone slot. An additional microdose requires explicit acknowledgement and stores `weeklyFrequencyOverride` plus its reason. Saved microdoses are classified as `skill_microdose`, remain outside primary/recovery counts, never alter the coach-directed run stage, and contribute only to weekly practice volume. There are no air squats, automatic scheduling, automatic progression, or medical/recovery recommendations.

Active standard Days 1–4 display a specific warm-up and exercises are numbered in intended completion order. Standard Day 1 keeps its full walk/run block after strength work because it is conditioning rather than the warm-up. The retired September 10 snapshot preserves its own return-to-exercise check only for historical rendering. Day 4 begins with its walk/run session after a separate walking and dynamic warm-up. Air squats are not in the current block.

Blank primary workouts select the day after the most recent rotation-advancing primary workout using session date and then `updatedAt`; recovery and skill microdoses are ignored. No illness-recovery option is active in v1.5.7. The saved September 10 entry still maps to Day 1 and the saved September 13 Day 2 entry advances the next automatic workout to standard Day 3. Dates use device-local components. **New Workout** and the post-save reset apply these defaults. Editing preserves the saved date, day, prescription, and rotation mapping.
...
The PWA uses versioned CSS/JavaScript/config URLs and network-first same-origin fetching with offline cache fallback. Version 1.5.7 uses service-worker cache **v50**. When a newly activated service worker takes control, a persistent **Reload update** banner appears. Reloading first autosaves the active draft.
