...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.6 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline. Versions 1.4.5 through 1.4.8 use **125 lb total for 2 × 8** and `foundation-1.4.5`, which preserves the same circuit work while adding the known M4 equipment and lane guidance. Versions 1.4.9 through 1.5.1 use **135 lb total for 2 × 8** without changing the circuit. The September 3, 2026 workout remains an immutable version 1.5.1 snapshot; version 1.5.2 changed only Day 2, so its Day 3 prescription would have been identical. Versions 1.5.3 through 1.5.6 use **145 lb total for 2 × 8** while retaining `foundation-1.4.5`. Never revise an older template or saved snapshot in place.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.6**, effective **2026-09-11**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.6 retires the active v1.5.5 **Day 1 — Illness Recovery** definition and selector option without creating a replacement or reusable template. Standard Days 1–4 and every other active prescription remain byte-for-byte equivalent to v1.5.5. The completed September 10 v1.5.5 workout remains authoritative through its immutable embedded snapshot, including `dayKey: day1IllnessRecovery`, `rotationDayKey: day1`, primary/advancing classification, stable `illnessReturnCheck` identity, shared exercise identities, and all saved results. Snapshot-backed retired session keys normalize and render without an active `SESSIONS` definition; the historical entry still exports, imports, persists locally, round-trips through Firebase, counts as primary Day 1, and leaves standard Day 2 as the next default. Global Stage 4, the capped Day 3 circuit, TANK M4 Level 3 metadata, optional recovery, and the auxiliary microdose remain unchanged. September 3 and September 5 remain immutable v1.5.1 and v1.5.3 snapshots, any v1.5.4 workout remains immutable, and version 1.5.2 was never installed in the public app. There is no automatic illness decision, weekday scheduling, third weekly run, or automatic progression.
...
Blank primary workouts select the day after the most recent rotation-advancing primary workout using session date and then `updatedAt`; recovery and skill microdoses are ignored. No illness-recovery option is active in v1.5.6, but the saved September 10 entry still maps to Day 1 for this calculation, so the next automatic workout is standard Day 2. Dates use device-local components. **New Workout** and the post-save reset apply these defaults. Editing preserves the saved date, day, prescription, and rotation mapping.
