...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.4.4 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.4.4**, effective **2026-08-19**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.4.4 leaves Day 1 exactly as prescribed in v1.4.3 and changes Day 2 only. Hand-release push-ups are 5 × 8, seated dumbbell overhead press is 25 lb per hand for 3 × 10, and the same-setup machine row is 88 lb for 3 × 12. Seated lat pulldown is preferred and uses the next same-machine increment above 143 lb—approximately 154 lb on the known 11-lb stack—for 3 × 8–10. Cable lateral raises use 2 × 15 on the same pain-free setup/load, and the preferred pec deck uses 66 lb for 2 × 15 on the same setup. Dead bug and 25–30 minutes of easy cardio remain unchanged. Days 3–4, Stage 3, and the auxiliary microdose remain unchanged. Machine and cable-stack cues without a tracked exact setup remain coaching guidance rather than universal load-adherence targets.
