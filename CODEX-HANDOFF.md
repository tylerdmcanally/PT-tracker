...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.0 retains exactly two conditioning rounds in this order:
...
`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.0**, effective **2026-08-29**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.0 changes Day 4 only: Stage 4 remains 1:00 walk / 2:30 run × 6, but the fresh-session treadmill cue now starts around 6.1–6.2 mph and moves toward 6.3–6.4 mph only while RPE remains at or below 6 and stride stays relaxed and normal; hand-release push-ups advance to 4 × 9. Day 4 front plank and mobility, Days 1–3, Stage 4, the progressive Day 3 warm-up, two-round circuit, known TANK M4 Level 3 metadata, and auxiliary microdose remain unchanged. There is no weekday scheduling, third weekly run, or automatic progression.
