...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.1 retains exactly two conditioning rounds in this order:
...
`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.1**, effective **2026-08-31**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.1 changes Day 1 only: trap-bar deadlift is 175 lb total for 3 × 5; leg press uses the next comparable increment above 140 lb for 3 × 8–10 on the same setup; seated cable row is 132 lb displayed for 3 × 10 on the same setup; EZ-bar preacher curl uses 30 lb total—or the next comparable load below 40 lb if 30 is unavailable—for 2 × 10–15; and cable pressdown uses the next comparable increment above 77 lb, approximately 88 displayed on the same 11-lb increment setup, for 2 × 10–12. Day 1 dumbbell bench, farmer carry, front plank, and post-lift Stage 4 run; Days 2–4; global Stage 4; the two-round circuit; known TANK M4 Level 3 metadata; and the auxiliary microdose remain unchanged. There is no weekday scheduling, third weekly run, or automatic progression.
