...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.4.2 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.2 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.4.2**, effective **2026-08-16**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.4.2 keeps all Day 1 and Day 2 strength prescriptions from v1.4.1. Day 3 progresses `oneArmRow` to 45 lb for 3 × 10 per side at RPE 6–8 and the dumbbell `hammerCurl` to 25 lb per hand for 2 × 10–15; the incline press, remaining strength work, and two-round circuit stay fixed. Day 4 keeps HRPU 4 × 7 and changes its front plank from 35/35/30 to 35/35/35 seconds. Machine- and cable-specific loads remain directly comparable only when the stored variation matches the target variation.
...
Run training defines 12 possible stages, but the current stage is read-only and coach-directed. Day 1 and Day 4 use the existing Stage 3: 1:00 walk / 2:00 run × 7, totaling 21 programmed minutes. Version 1.4.2 guides treadmill running segments to start around 6.2 mph, remain within 6.2–6.4 mph, and target RPE 5–6 while leaving roughly 30–60 seconds of running available after each interval. The existing walk/run speed fields record actual speeds; speed and average pace do not alter structured interval adherence. A user can still record a different actual stage inside a session. Completion counts are informational and never advance or regress the program.
