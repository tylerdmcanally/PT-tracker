...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.3 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline. Versions 1.4.5 through 1.4.8 use **125 lb total for 2 × 8** and `foundation-1.4.5`, which preserves the same circuit work while adding the known M4 equipment and lane guidance. Versions 1.4.9 through 1.5.1 use **135 lb total for 2 × 8** without changing the circuit. The September 3, 2026 workout remains an immutable version 1.5.1 snapshot; version 1.5.2 changed only Day 2, so its Day 3 prescription would have been identical. Version 1.5.3 advances only the active Day 3 Romanian-deadlift prescription to **145 lb total for 2 × 8** while retaining `foundation-1.4.5`. Never revise an older template or saved snapshot in place.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.3**, effective **2026-09-04**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.3 is a cumulative synchronization from the public app's version 1.5.1 baseline. Day 2 advances hand-release push-ups to 5 × 10, the seated pulldown to the next same-machine increment above 165 lb, seated dumbbell overhead press to 30 lb per hand for 3 × 9, machine row to the next same-machine increment above 99 lb, cable lateral raise to 33 lb displayed per side for 2 × 20, and pec deck to 77 lb displayed for 2 × 15; easy cardio remains 25–30 minutes at RPE 4–5 and dead bug remains unchanged. Day 3 advances Romanian deadlift to 145 lb total for 2 × 8, incline dumbbell press to 35 lb per hand for 3 × 9, one-arm dumbbell row to 50 lb for 3 × 8–10 per side, hammer curl to 25 lb per hand for 2 × 12, and overhead cable triceps extension to the next same-setup increment above 99 lb. Days 1 and 4, global Stage 4, the capped two-round circuit, TANK M4 Level 3 metadata, and the auxiliary microdose remain unchanged. Version 1.5.2 was never installed in the public app and must not be synthesized as a current or historical app version. There is no weekday scheduling, third weekly run, or automatic progression.
