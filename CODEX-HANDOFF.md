...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.5 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline. Versions 1.4.5 through 1.4.8 use **125 lb total for 2 × 8** and `foundation-1.4.5`, which preserves the same circuit work while adding the known M4 equipment and lane guidance. Versions 1.4.9 through 1.5.1 use **135 lb total for 2 × 8** without changing the circuit. The September 3, 2026 workout remains an immutable version 1.5.1 snapshot; version 1.5.2 changed only Day 2, so its Day 3 prescription would have been identical. Versions 1.5.3 through 1.5.5 use **145 lb total for 2 × 8** while retaining `foundation-1.4.5`. Never revise an older template or saved snapshot in place.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.5**, effective **2026-09-10**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.5 preserves standard Days 1–4 exactly as defined in v1.5.4 and adds one manually selected coach-directed alternative, **Day 1 — Illness Recovery**. It is a primary session with `rotationDayKey: day1`, so saving it advances the standard rotation to Day 2, but it is never part of `PROGRAM.rotation` or selected automatically. Its capped 45-minute prescription uses a 10-minute easy return check followed by reduced-volume Day 1 deadlift, leg press, dumbbell bench press, seated cable row, farmer carry, and front-plank work at no higher than RPE 6; it contains no run or arm accessories. The alternative reuses shared exercise IDs and preserves equipment comparability, while `illnessReturnCheck` remains distinct from recovery cardio. Global Stage 4, the capped Day 3 circuit, TANK M4 Level 3 metadata, optional recovery, and the auxiliary microdose remain unchanged. September 3 and September 5 remain immutable v1.5.1 and v1.5.3 snapshots, any v1.5.4 workout remains immutable, and version 1.5.2 was never installed in the public app. There is no automatic illness decision, weekday scheduling, third weekly run, or automatic progression.
