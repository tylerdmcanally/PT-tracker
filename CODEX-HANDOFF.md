...
Day 3 is **Lower Strength and Gym Conditioning**. Version 1.5.4 retains exactly two conditioning rounds in this order:
...
Historical version 1.3 Day 3 snapshots retain the 95 lb Romanian-deadlift target, the four-component `foundation-1.2` circuit, and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use **115 lb total for 2 × 8** and the six-component `foundation-1.4` circuit as their baseline. Versions 1.4.5 through 1.4.8 use **125 lb total for 2 × 8** and `foundation-1.4.5`, which preserves the same circuit work while adding the known M4 equipment and lane guidance. Versions 1.4.9 through 1.5.1 use **135 lb total for 2 × 8** without changing the circuit. The September 3, 2026 workout remains an immutable version 1.5.1 snapshot; version 1.5.2 changed only Day 2, so its Day 3 prescription would have been identical. Versions 1.5.3 and 1.5.4 use **145 lb total for 2 × 8** while retaining `foundation-1.4.5`. Never revise an older template or saved snapshot in place.

## Current program model

`program-config.js` is the single source of truth for program metadata, daily prescriptions, stable exercise IDs, target RPEs, coaching notes, run stage, recovery work, and auxiliary templates. The active primary program is **AFT Foundation Block 1**, version **1.5.4**, effective **2026-09-06**. The independent auxiliary template is **AFT Skill Microdose**, version **1.0**, effective **2026-08-06**.

Version 1.5.4 changes only the Stage 4 pace guidance on Days 1 and 4 plus the Day 4 front-plank target. The post-lift Day 1 run now begins around 5.5–5.6 mph and may move toward 5.7–5.8 only while RPE remains at or below 6. The fresh Day 4 run begins around 5.7–5.8 mph, may move toward 5.9–6.0 only if RPE is at or below 5 through round 3, and reduces toward 5.5 if RPE exceeds 6. Day 4 front plank advances from 3 × 45 seconds to 3 × 50 seconds with clean position required. Global Stage 4 remains 1:00 walk / 2:30 run × 6 and all other Days 1–4 prescriptions, the capped two-round circuit, TANK M4 Level 3 metadata, optional recovery, and the auxiliary microdose remain unchanged. The September 5 workout remains an immutable version 1.5.3 snapshot. Version 1.5.2 was never installed in the public app and must not be synthesized as a current or historical app version. There is no weekday scheduling, third weekly run, or automatic progression.
...
Run training defines 12 possible stages, but the current stage is read-only and coach-directed. Version 1.5.4 holds active Stage 4 at 1:00 walk / 2:30 run × 6: six minutes walking plus fifteen minutes running for 21 programmed minutes. The post-strength Day 1 running segments begin around 5.5–5.6 mph, stay controlled through at least the first three rounds, and may move toward 5.7–5.8 mph only while RPE remains at or below 6 with relaxed stride and breathing. The fresh-leg Day 4 running segments begin around 5.7–5.8 mph and hold that range through at least round 3; move toward 5.9–6.0 only if RPE is at or below 5 through round 3 and reduce toward 5.5 if RPE exceeds 6. Do not advance beyond Stage 4 until two pain-free completions under this guidance land at RPE 5–6, including one fresh Day 4 without a late forced slowdown. Both runs prioritize duration and reserve rather than speed, total distance, or average pace. Every round begins with walking. The existing walk/run speed fields record actual speeds; completion never advances or regresses the program automatically. Historical workouts retain their saved prescription, guidance, and result fields.
