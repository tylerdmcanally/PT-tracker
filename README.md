...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.5.6 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use a 115 lb total Romanian-deadlift target for 2 × 8 and the original sled-enhanced `foundation-1.4` circuit. Versions 1.4.5 through 1.4.8 use 125 lb for 2 × 8 and a separately versioned circuit template for the known M4 setup. Versions 1.4.9 through 1.5.1 use 135 lb for 2 × 8 without changing the two-round circuit. The September 3 workout remains an immutable v1.5.1 snapshot. Versions 1.5.3 through 1.5.6 use 145 lb for 2 × 8 while retaining the same circuit. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.5.6**, effective September 11, 2026. Version 1.5.6 retires the v1.5.5 **Day 1 — Illness Recovery** definition and selector option without replacing it; standard Days 1–4 and every other active prescription remain unchanged. The completed September 10 v1.5.5 session remains historical data backed by its immutable snapshot and `rotationDayKey: day1`, so it continues to load, edit, export, sync, count as primary, and leave standard Day 2 as the next default. September 3 and September 5 remain immutable v1.5.1 and v1.5.3 history, any v1.5.4 workout remains immutable, and version 1.5.2 was never installed in the public app.
