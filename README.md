...
- Four primary workout days, a manually selected coach-directed Day 1 illness-recovery alternative, and separate optional recovery and AFT Skill Microdose sessions
...
- Automatic Day 1 → Day 4 rotation that maps a saved illness-recovery alternative to Day 1 and ignores recovery and skill-microdose sessions
...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.5.5 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use a 115 lb total Romanian-deadlift target for 2 × 8 and the original sled-enhanced `foundation-1.4` circuit. Versions 1.4.5 through 1.4.8 use 125 lb for 2 × 8 and a separately versioned circuit template for the known M4 setup. Versions 1.4.9 through 1.5.1 use 135 lb for 2 × 8 without changing the two-round circuit. The September 3 workout remains an immutable v1.5.1 snapshot. Versions 1.5.3 through 1.5.5 use 145 lb for 2 × 8 while retaining the same circuit. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.5.5**, effective September 10, 2026. Standard Days 1–4 remain unchanged from v1.5.4, including global Stage 4, both run pace guides, and the Day 4 front-plank target. Version 1.5.5 adds the separately selectable **Day 1 — Illness Recovery** session: a coach-directed, reduced-volume alternative that is never selected automatically, contains no run or accessory work, and advances the primary rotation as Day 1 when saved. The September 3 and September 5 workouts remain immutable v1.5.1 and v1.5.3 history, any v1.5.4 workout remains immutable, and version 1.5.2 was never installed in the public app.
...
New workouts use the device-local calendar date and default to the day following the most recent saved primary session. The illness-recovery alternative is available only through the coach-directed alternatives selector group; saving it advances to standard Day 2, but it never appears as an automatic default. Optional recovery and skill-microdose sessions appear separately in history and exports but do not advance that rotation. The **New Workout** action returns to the suggested standard primary day without affecting saved history.

The separately versioned **AFT Skill Microdose v1.0** is available at most once per Monday–Sunday week under normal use. It adds low-fatigue hand-release push-up and front-plank practice without advancing the primary rotation or changing the coach-directed run stage. It is the sole optional extra push-up/plank practice in program v1.5.5; historical v1.3 Day 3 skill bundles continue to retain their original weekly-dose behavior. Completed microdose reps and front-plank seconds count only as AFT-event practice volume, never as benchmark results.

Run progression is coach-directed rather than automatically advanced. Program v1.5.5 holds Stage 4—one minute walking followed by two minutes thirty seconds running for six rounds, totaling 21 programmed minutes—on standard Days 1 and 4. The post-strength Day 1 run starts around 5.5–5.6 mph and may move toward 5.7–5.8 only while RPE remains at or below 6. The fresh-leg Day 4 run starts around 5.7–5.8 mph and may move toward 5.9–6.0 only if RPE is at or below 5 through round 3; reduce toward 5.5 if RPE exceeds 6. Both prioritize duration and reserve over speed or average pace. Stage 4 does not advance until two pain-free completions under this guidance land at RPE 5–6, including one fresh Day 4 without a late forced slowdown. The illness-recovery alternative contains no run. The timer starts every interval round with walking, automatically logs completed rounds and programmed interval time, and marks the run exercise done. Total elapsed time remains a separate user-entered value and is not used to judge structured interval adherence.
...
The full standard Day 1 walk/run interval block remains after the primary strength work because it is conditioning, not the warm-up. Standard Days 1–4 display a separate active warm-up; the illness-recovery alternative instead begins with its prescribed 10-minute return-to-exercise check. Day 4 starts with its primary run after its warm-up, and every walk/run timer begins with the walk segment before progressing to the run segment.
