...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.5.1 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use a 115 lb total Romanian-deadlift target for 2 × 8 and the original sled-enhanced `foundation-1.4` circuit. Versions 1.4.5 through 1.4.8 use 125 lb for 2 × 8 and a separately versioned circuit template for the known M4 setup. Version 1.4.9 advances only the active Romanian-deadlift target to 135 lb for 2 × 8 without changing the two-round circuit. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.5.1**, effective August 31, 2026. Day 1 advances trap-bar deadlift to 175 lb total for 3 × 5; leg press to the next comparable increment above 140 lb for 3 × 8–10 on the same setup; seated cable row to 132 lb displayed for 3 × 10 on the same setup; EZ-bar preacher curl to 30 lb total—or the next comparable load below 40 lb if 30 is unavailable—for 2 × 10–15; and cable pressdown to the next comparable increment above 77 lb, approximately 88 displayed on the same 11-lb increment setup, for 2 × 10–12. Day 1 dumbbell bench, farmer carry, front plank, and post-lift run; Days 2–4; the four-day rotation; Stage 4; the two-round conditioning circuit; and the skill microdose remain unchanged.
