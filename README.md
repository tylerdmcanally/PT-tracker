...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.4.2 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.2 use a 115 lb total Romanian-deadlift target for 2 × 8 and the sled-enhanced six-component circuit as their baseline. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.4.2**, effective August 16, 2026. It retains all Day 1 and Day 2 strength work from v1.4.1, progresses the Day 3 one-arm dumbbell row to 45 lb for 3 × 10 per side and the dumbbell hammer curl to 25 lb per hand, and sets Day 4 front planks to 3 × 35 seconds. Run Stage 3 remains active with slower treadmill guidance—start around 6.2 mph, work within 6.2–6.4 mph, and target RPE 5–6—to build continuous-running tolerance before speed. Recovery, the skill microdose, and the Day 3 conditioning circuit remain unchanged.
...
The separately versioned **AFT Skill Microdose v1.0** is available at most once per Monday–Sunday week under normal use. It adds low-fatigue hand-release push-up and front-plank practice without advancing the primary rotation or changing Run Stage 3. It is the sole optional extra push-up/plank practice in program v1.4.2; historical v1.3 Day 3 skill bundles continue to retain their original weekly-dose behavior. Completed microdose reps and front-plank seconds count only as AFT-event practice volume, never as benchmark results.

Run progression is coach-directed rather than automatically advanced. Program v1.4.2 keeps Stage 3—one minute walking followed by two minutes running for seven rounds, totaling 21 programmed minutes—on Days 1 and 4. Treadmill running segments start around 6.2 mph, stay within an acceptable 6.2–6.4 mph range, and target RPE 5–6; speed is coaching guidance and does not alter structured adherence. The timer starts every interval round with walking, automatically logs completed rounds and programmed interval time, and marks the run exercise done. Total elapsed time remains a separate user-entered value and is not used to judge structured interval adherence.
