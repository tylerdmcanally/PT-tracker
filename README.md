...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.5.11 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use a 115 lb total Romanian-deadlift target for 2 × 8 and the original sled-enhanced `foundation-1.4` circuit. Versions 1.4.5 through 1.4.8 use 125 lb for 2 × 8 and a separately versioned circuit template for the known M4 setup. Versions 1.4.9 through 1.5.1 use 135 lb for 2 × 8 without changing the two-round circuit. The September 3 workout remains an immutable v1.5.1 snapshot. Versions 1.5.3 through 1.5.7 use 145 lb for 2 × 8 while retaining the same circuit. Versions 1.5.9 through 1.5.11 Strength 3 use 155 lb for 2 × 8 and keep that circuit unchanged. Public v1.5.8 was never installed or synthesized. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.5.11**, effective September 21, 2026, with service-worker cache **v55** and data schema 11. Its ordered primary rotation is **Strength 1 — Upper Body and AFT Calisthenics**, **Strength 2 — Heavy Strength and Carries**, and **Strength 3 — Lower / Full Body and SDC**. Version 1.5.9 introduced six dedicated session containers, and v1.5.10 changed only the active Strength 1 seated dumbbell overhead press to 25 lb per hand for 3 × 8–10 at RPE 7–8. Version 1.5.11 preserves every strength prescription and removes Run 1, Aerobic Base, and Run 2 only from active selection; their exact v1.5.10 definitions remain available for historical records and in-progress legacy drafts. Runna separately owns current running prescription, tracking, and progression; this PWA does not integrate with Runna. The press remains third after hand-release push-ups and pulldowns, each set is capped at 10 clean submaximal repetitions, and returning to 30 lb requires a later coach-directed version after a clean 3 × 10 at no more than RPE 8. Strength 3 carries the cumulative coach-directed changes: Romanian deadlift 155 lb for 2 × 8, one-arm row 50 lb for 3 × 9 per side, tempo body-weight split squat 2 × 12 per leg, 50-second front and side planks, hammer curl 25 lb per hand for 2 × 15, and same-setup overhead cable triceps extension 110 displayed for 2 × 12. Goblet squat, incline press, the two-round circuit, and all TANK metadata remain constrained as prescribed.

The completed September 13 Day 2 workout remains immutable v1.5.6 history, including its saved prescription snapshot, results, RPE, pain, and notes; active v1.5.11 targets never rewrite it. September 16 Run 2 and September 18 Strength 1 likewise remain immutable v1.5.9 history with their original snapshots and results, and September 20 Run 1 remains immutable v1.5.10 history. Version 1.5.6 retired the v1.5.5 **Day 1 — Illness Recovery** definition and selector option without replacing it. The completed September 10 v1.5.5 session remains historical data backed by its immutable snapshot and `rotationDayKey: day1`, so it continues to load, edit, export, sync, and count as primary. Legacy Day 1–Day 4 entries, including v1.5.7 Day 3 history, remain immutable and retain their saved primary classification even though those keys are no longer active. September 3 and September 5 remain immutable v1.5.1 and v1.5.3 history, any v1.5.4 workout remains immutable, and versions 1.5.2 and 1.5.8 were never installed or synthesized in the public app.
