...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.4.4 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use a 115 lb total Romanian-deadlift target for 2 × 8 and the sled-enhanced six-component circuit as their baseline. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.4.4**, effective August 19, 2026. Day 1 remains unchanged from v1.4.3. Day 2 advances hand-release push-ups to 5 × 8, seated dumbbell overhead press to 25 lb per hand for 3 × 10, and the same-setup machine row to 88 lb for 3 × 12. The preferred seated lat pulldown uses the next same-machine increment above 143 lb for 3 × 8–10, cable lateral raises use 2 × 15 on the same pain-free setup/load, and the preferred pec deck uses 66 lb for 2 × 15 on the same setup. Machine stack values remain setup-specific coaching guidance rather than universal load-adherence targets. Days 3–4, Stage 3, the skill microdose, and the two-round conditioning circuit remain unchanged.
...
The separately versioned **AFT Skill Microdose v1.0** is available at most once per Monday–Sunday week under normal use. It adds low-fatigue hand-release push-up and front-plank practice without advancing the primary rotation or changing Run Stage 3. It is the sole optional extra push-up/plank practice in program v1.4.4; historical v1.3 Day 3 skill bundles continue to retain their original weekly-dose behavior. Completed microdose reps and front-plank seconds count only as AFT-event practice volume, never as benchmark results.

Run progression is coach-directed rather than automatically advanced. Program v1.4.4 keeps Stage 3—one minute walking followed by two minutes running for seven rounds, totaling 21 programmed minutes—on Days 1 and 4. The post-strength Day 1 run starts around 6.0–6.2 mph, while the fresher Day 4 assessment retains its approximately 6.2 mph start and 6.2–6.4 mph working range. Both target RPE 5–6; speed is coaching guidance and does not alter structured adherence. The timer starts every interval round with walking, automatically logs completed rounds and programmed interval time, and marks the run exercise done. Total elapsed time remains a separate user-entered value and is not used to judge structured interval adherence.
