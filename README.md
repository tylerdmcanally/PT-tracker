...
- A Monday–Sunday skill-dose rule with the standalone microdose as the sole optional extra in program v1.5.3 while historical v1.3 Day 3 bundles remain compatible
...
Historical version 1.3 snapshots retain the original four-component circuit and any saved one-occurrence sled directive. Versions 1.4 through 1.4.4 use a 115 lb total Romanian-deadlift target for 2 × 8 and the original sled-enhanced `foundation-1.4` circuit. Versions 1.4.5 through 1.4.8 use 125 lb for 2 × 8 and a separately versioned circuit template for the known M4 setup. Versions 1.4.9 through 1.5.1 use 135 lb for 2 × 8 without changing the two-round circuit. The September 3 workout remains an immutable v1.5.1 snapshot. Version 1.5.3 advances only the active Romanian-deadlift target to 145 lb for 2 × 8 while retaining the same circuit. Visible target loads are never prefilled as completed results.

The active primary program is **AFT Foundation Block 1 v1.5.3**, effective September 4, 2026. This cumulative synchronization preserves the existing v1.5.1 Day 1 prescription, applies the pending Day 2 changes—5 × 10 hand-release push-ups, higher same-setup pulldown and machine-row increments, 30 lb-per-hand overhead press for 3 × 9, 33 displayed-per-side lateral raises for 2 × 20, and 77 displayed pec deck for 2 × 15—and advances selected Day 3 work: 145 lb Romanian deadlift for 2 × 8, 35 lb-per-hand incline press for 3 × 9, 50 lb one-arm rows for 3 × 8–10 per side, 25 lb-per-hand hammer curls for 2 × 12, and the next same-setup overhead-triceps increment above 99. Days 1 and 4, the four-day rotation, Stage 4, the two-round conditioning circuit, and the skill microdose remain unchanged. Version 1.5.2 was never installed in the public app.
...
The separately versioned **AFT Skill Microdose v1.0** is available at most once per Monday–Sunday week under normal use. It adds low-fatigue hand-release push-up and front-plank practice without advancing the primary rotation or changing the coach-directed run stage. It is the sole optional extra push-up/plank practice in program v1.5.3; historical v1.3 Day 3 skill bundles continue to retain their original weekly-dose behavior. Completed microdose reps and front-plank seconds count only as AFT-event practice volume, never as benchmark results.

Run progression is coach-directed rather than automatically advanced. Program v1.5.3 holds Stage 4—one minute walking followed by two minutes thirty seconds running for six rounds, totaling 21 programmed minutes—on Days 1 and 4. The post-strength Day 1 run begins around 6.0–6.1 mph. The fresh-leg Day 4 run starts around 6.1–6.2 mph and moves toward 6.3–6.4 mph only while RPE remains at or below 6 and stride stays relaxed and normal. Both prioritize longer continuous-running segments over speed or average pace. The timer starts every interval round with walking, automatically logs completed rounds and programmed interval time, and marks the run exercise done. Total elapsed time remains a separate user-entered value and is not used to judge structured interval adherence.
